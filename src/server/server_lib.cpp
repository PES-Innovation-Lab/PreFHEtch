...
std::vector<std::vector<float>> Server::deserialise_decrypt_coarse_distances(
    const std::vector<std::vector<std::vector<seal::seal_byte>>>
        &serde_encrypted_coarse_distances,
    // TODO: Temp Param
    ...
    // TODO: TEMP
    seal::SecretKey sk;
    sk.load(seal_ctx, serde_sk.data(), serde_sk.size());
    seal::BatchEncoder be(seal_ctx);
    seal::Decryptor dec(seal_ctx, sk);
    ...
    for (int i = 0; i < serde_encrypted_coarse_distances.size(); i++) {
        ...
        for (int j = 0; j < serde_encrypted_coarse_distances[i].size(); j++) {
            ...
            dec.decrypt(encrypted_coarse_distance, decrypted_coarse_distance);
            be.decode(decrypted_coarse_distance, decoded_coarse_distances);
...
Server::compute_nearest_coarse_vectors_idx(
